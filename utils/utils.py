...
import torch
import numpy as np
from scipy import misc  # 图像保存读取以及处理的包


def load_test_data(image_path, size=256):   #  Mask * image
...
def cam(x, size=256):
    x = x - np.min(x)
    cam_img = x / (np.max(x) + 0.00001)
    cam_img = np.uint8(255.0 * cam_img)
    cam_img = cv2.resize(cam_img, (size, size))
    cam_img = cv2.applyColorMap(cam_img, cv2.COLORMAP_JET)
    return cam_img / 255.0


def imagenet_norm(x):  # 三维通道
