...
curPath = os.path.abspath(os.path.dirname(__file__))   # os.path.dirname(__file__)去掉文件名，返回目录
...
class FaceSeg:
    ...
    def get_mask(self, image):
        output = self._sess.run(self.output_op, feed_dict={self.input_op: image})[0]
        return output
