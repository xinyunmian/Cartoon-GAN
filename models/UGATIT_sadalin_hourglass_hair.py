...
from FP.parameters import get_parameters
from FP.parsing_usage import Parsing
...
class UgatitSadalinHourglass(object):
    ...
    def train(self):
        ...
        for step in range(self.start_iter, self.iteration + 1):
            ...
            tester = Parsing(config)
            hair_real_A = tester.test(real_A)
            hair_fake_A2B = tester.test(fake_A2B)
            feature_real_A = vgg(hair_real_A)
            feature_fake_A2B = vgg(hair_fake_A2B)
            features_x = vgg(real_A)
            features_y = vgg(fake_A2B)
            ...
            # G_tv_total_loss = self.TVLoss(fake_A2B) + self.TVLoss(fake_B2B) + self.TVLoss(fake_B2A2B)
            # identity loss
            ...
            content_hair = self.MSE_loss(feature_real_A.relu4_3, feature_fake_A2B.relu4_3)
           # content_3 = self.MSE_loss(features_y_reverse.relu4_3, features_x.relu4_3)
            ...
            content_loss = 0.1 * content_1 + 0.05 * content_2 + content_hair
