...
#from .usage_segment import segment
...
class UgatitSadalinHourglass(object):
    ...
    def build_model(self):
        ...
        train_transform = transforms.Compose([
            ...
            transforms.RandomAffine(20, fillcolor=(255, 255, 255), resample=Image.BILINEAR),
            transforms.ToTensor(),
        ...
        self.facenet = FaceFeatures('models/model_ir_se50.pth', self.device)
        ...
        # self.MS_ssim_loss = LPIPS(net_type='alex').to(self.device).eval()
        self.MS_ssim_loss = MS_SSIM(max_val=1)
        # self.MS_ssim_loss = lpips.LPIPS(net='alex').to(self.device)
        self.MS_ssim_w = 0.84
        self.lpips_loss = lpips.LPIPS(net='alex').to(self.device)
        """ Trainer """
        self.G_optim = Ranger(itertools.chain(self.genA2B.parameters(), self.genB2A.parameters()), lr=self.lr)
        self.D_optim = Ranger(
    ...
    def train(self):
        ...
        for step in range(self.start_iter, self.iteration + 1):
            ...
            real_GA_logit, real_GA_cam_logit, _ = self.disGA(DiffAugment(real_A, policy=self.policy))
            real_LA_logit, real_LA_cam_logit, _ = self.disLA(DiffAugment(real_A, policy=self.policy))
            real_GB_logit, real_GB_cam_logit, _ = self.disGB(DiffAugment(real_B, policy=self.policy))
            real_LB_logit, real_LB_cam_logit, _ = self.disLB(DiffAugment(real_B, policy=self.policy))

            fake_GA_logit, fake_GA_cam_logit, _ = self.disGA(DiffAugment(fake_B2A, policy=self.policy))
            fake_LA_logit, fake_LA_cam_logit, _ = self.disLA(DiffAugment(fake_B2A, policy=self.policy))
            fake_GB_logit, fake_GB_cam_logit, _ = self.disGB(DiffAugment(fake_A2B, policy=self.policy))
            fake_LB_logit, fake_LB_cam_logit, _ = self.disLB(DiffAugment(fake_A2B, policy=self.policy))

            # Least Square Loss
            # real label 1, fake 0, cam to discrimiante real or fake
            ...
            #Gradient_penalty_A = self.gradient_penalty_A(real_A, fake_B2A)
            #Gradient_penalty_B = self.gradient_penalty_B(real_B, fake_A2B)

            D_loss_A = self.adv_weight * (D_ad_loss_GA + D_ad_cam_loss_GA + D_ad_loss_LA + D_ad_cam_loss_LA) #+ 5 * Gradient_penalty_A
            D_loss_B = self.adv_weight * (D_ad_loss_GB + D_ad_cam_loss_GB + D_ad_loss_LB + D_ad_cam_loss_LB) #+ 5 * Gradient_penalty_B
            ...
            #mask = -torch.ones(220, 256).to(self.device)
            #pad = nn.ConstantPad2d(padding=(0, 0, 0, 36), value=1) 
            #mask = pad(mask)
            features_x = self.vgg(real_A)
            features_y = self.vgg(fake_A2B)
            #features_y_reverse = vgg(fake_A2B2A)

            #seg_1 = segment(fake_A2B)
            #seg_2 = segment(real_A)
            ...
            # rec loss
            G_recon_loss_A = self.MS_ssim_w * (1-self.MS_ssim_loss(fake_A2B2A, real_A)) + (1 - self.MS_ssim_w) * self.L1_loss(fake_A2B2A, real_A)
            # G_recon_loss_A = 0.5 * self.lpips_loss(real_A, fake_A2B2A)
            G_recon_loss_B = self.MS_ssim_w * (1-self.MS_ssim_loss(fake_B2A2B, real_B)) + (1 - self.MS_ssim_w) * self.L1_loss(fake_B2A2B, real_B)

            
            # identity loss
            G_identity_loss_A = self.MS_ssim_w * (1-self.MS_ssim_loss(fake_A2A, real_A)) + (1 - self.MS_ssim_w) * self.L1_loss(fake_A2A, real_A)
            G_identity_loss_B = self.MS_ssim_w * (1-self.MS_ssim_loss(fake_B2B, real_B)) + (1 - self.MS_ssim_w) * self.L1_loss(fake_B2B, real_B)
            ...
            # seg loss
            #seg_loss = 1.0 * self.MSE_loss(seg_1, seg_2)
            # content loss
            content_1 = self.MSE_loss(features_y.relu4_3, features_x.relu4_3)
            content_2 = self.MSE_loss(features_y.relu3_3, features_x.relu3_3)
           # content_3 = self.MSE_loss(features_y_reverse.relu4_3, features_x.relu4_3)
            #content_4 = self.MSE_loss(features_y_reverse.relu3_3, features_x.relu3_3)

            content_loss = 0.2 * content_1 + 0.05 * content_2
            ...
            G_loss_A = self.adv_weight * (G_ad_loss_GA + G_ad_cam_loss_GA + G_ad_loss_LA + G_ad_cam_loss_LA) + \
                       ...
                       self.cam_weight * G_cam_loss_A + self.faceid_weight * G_id_loss_A + content_loss #+ seg_loss
    ...
    def gradient_penalty_A(self, input_img, generate_img):
        # interpolate sample
        ...
        inter_img_prob_0, _, _ = self.disGA(DiffAugment(inter_img, policy=self.policy))
        inter_img_prob_1, _, _ = self.disLA(DiffAugment(inter_img, policy=self.policy))
        for i in range(0, 2):
        # computer gradient penalty: x: inter_img, y: inter_img_prob
        # (L2_norm(dy/dx) - 1)**2
            dydx_l2norm = 0
            if i ==0:
                inter_img_prob = inter_img_prob_0
            else:
                inter_img_prob = inter_img_prob_1
            ...
            dydx_l2norm_i = torch.sqrt(torch.sum(dydx ** 2, dim=1))
            dydx_l2norm_i = torch.mean(dydx_l2norm_i ** 2)
            dydx_l2norm += dydx_l2norm_i

        return dydx_l2norm

    def gradient_penalty_B(self, input_img, generate_img):
        # interpolate sample
        alpha = torch.rand(input_img.size(0), 1, 1, 1).to(self.device)
        inter_img = (alpha * input_img.data + (1 - alpha) * generate_img.data).requires_grad_(True)
        inter_img_prob_0, _, _ = self.disGB(DiffAugment(inter_img, policy=self.policy))
        inter_img_prob_1, _, _ = self.disLB(DiffAugment(inter_img, policy=self.policy))
        for i in range(0, 2):
        # computer gradient penalty: x: inter_img, y: inter_img_prob
        # (L2_norm(dy/dx) - 1)**2
            dydx_l2norm = 0
